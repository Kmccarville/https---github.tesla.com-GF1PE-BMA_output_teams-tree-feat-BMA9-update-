...
def main(env,eos=False):
    ...
    if env == 'prod':
        ...
        try:
            historize_to_db(teams_con, 14, hourly_goal_dict['3BM4'], *cta4_outputs, np.sum(cta4_outputs), CTA_DIVISOR)
            historize_to_db(teams_con, 15, hourly_goal_dict['3BM5'], *cta5_outputs, np.sum(cta5_outputs), CTA_DIVISOR)
            historize_to_db(teams_con, 16, None, *cta6_outputs, np.sum(cta6_outputs), CTA_DIVISOR)
            historize_to_db(teams_con, 17, None, *cta7_outputs, np.sum(cta7_outputs), CTA_DIVISOR)
            historize_to_db(teams_con, 19, None, *cta9_outputs, np.sum(cta9_outputs), CTA_DIVISOR)
